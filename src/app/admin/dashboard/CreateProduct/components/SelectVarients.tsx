                ...
                <div  className='flex flex-col gap-3 lg:w-full w-[85%] p-5 bg-stone-800/50 rounded-lg border border-amber-200/20 !text-amber-200/50 mx-auto ' >
                    ...
                    {/* Options inputs */}
                        <div className='flex justify-between w-full '>
                            <input type="number" name="price" value={FeildVariants.price ?? ''}  placeholder="Price"  onChange={(e) => setFeildVariants( ( pre : any ) => ({...pre , price: Number(e.target.value)}))}  className="input w-[40%]" />
                            <input type="number" name="inventory" value={FeildVariants.inventory ?? ''}  placeholder="inventory" onChange={(e) => setFeildVariants( ( pre : any ) => ({...pre , inventory: Number(e.target.value)}))}  className="input w-[40%]" />
                        </div>
                        <div className='flex justify-between'>
                            <button type='button'  className={`${FormButn}`} onClick={() => setKeyNumber( KeyNumber + 1 )} >Add new Key</button>
                            <button type='button'  onClick={() => { if( FeildVariants.options.length > 1) {FeildVariants.options.pop() ; setKeyNumber( KeyNumber - 1 )} }  } className={`${FormButn} ${FeildVariants.options.length > 1 ? 'opacity-[1]' : 'opacity-0'} !duration-1000`} >Remove the Key</button>
                        </div>
