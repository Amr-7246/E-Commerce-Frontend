                ...
                {isLoading || isFetching ? ( <div> <Loading/> </div> ) : isError || (products ?? []).length < 0 ?  ( <p className="text-red-400 font-semibold"> Error loading products : </p> ) : (
                    !isLoading && !isFetching && (products ?? []).length > 0 && (
                        ...
                        <div className=" flex-center flex-col gap-5">
                            ...
                            <>
                                <div key={idx}  className='flex gap-5 flex-col '>
                                    <div className='lg:flex-row flex flex-col'>
                                        ...
                                        {/* Config buttons */}
                                            <div className=' flex items-end justify-end gap-3 p-4' >
                                                ...
                                                <button className='btn h-fit '  onClick={() => { if(WhoEditing == product._id ) {setWhoEditing('')}else {setWhoEditing(product._id)} }} >{ WhoEditing == product._id ? 'Close' : 'Edit Product'}</button>
                                            </div>
                                        {/* Config buttons */}
                                    </div>
                                    {/* Edittor form */}
