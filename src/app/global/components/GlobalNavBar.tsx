...
import { useLogOut } from '../user/Auth/logOut';
            ...
            <div className='flex-center lg:gap-8 gap-4 text-[var(--text-secondary)] '>
                {options.map((option, idx) => (
                    ...
                    <div className='flex justify-between items-center gap-2' key={idx}>
                        <Link
                            className={`${curentPath == (option.fake_href) ? '!text-[var(--text-primary)]' : ''} hover:!text-[var(--text-primary)] duration-500 cursor-pointer`} key={idx}
                            href={option.href}>
                            {option.name}
                        </Link>
