...
// ~ ########## Logics
    ...
    <div className=" w-full flex-center flex-col gap-5 ">
        {/* Slider Header */}
            <div className='w-[78%] md:w-[83%] flex-center gap-2'>
                <div className=' hidden md:flex md:flex-1 bg-black/50 border-amber-200/20 flex-center rounded-xl p-5 border-[1px]'>
                    <span  className="text-[20px] font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-200/50 via-orange-900 to-amber-200/50" >{title}</span>
                </div>
                <Link onClick={() => setWhichCatigory(category) } href={'/global/store/AllProducts'}  className='md:w-fit w-[100%] cursor-pointer hover:bg-transparent duration-800 bg-black/50 border-amber-200/20 flex-center rounded-xl p-5 border-[1px]'>
                    <span  className="text-[20px] font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-200/50 via-orange-900 to-amber-200/50" >Go to {title} section </span>
                </Link>
            </div>
        {/* Slider Header */}
        {/* Slider itself */}
            <div className=" flex-center  overflow-hidden w-[95%] mx-auto p-4 relative">
                {isLoading ? ( <div className="grid col-span-3"> <Loading /> </div> ) : isError ? ( <div className="grid col-span-3"> <p className="text-orange-600 font-black text-xl"> Sorry bro, something went wrong </p> </div> ) : (
                    <>
                        <div ref={sliderRef} className="flex  rounded-xl w-[88%] overflow-x-hidden scroll-smooth snap-x snap-mandatory space-x-4 pb-6" >
                            {products?.map((product : any , idx : any ) => (
                                <div key={idx} className="snap-center flex-shrink-0 w-[300px] bg-black/50 rounded-2xl overflow-hidden shadow-lg p-4 space-y-3 hover:shadow-amber-200/20 transition-all duration-300" >
                                    {/* Product Deets */}
                                        <img 
                        ...
                        {/* ARROW BUTTONS */}
                            <button className="absolute cursor-pointer left-[0] md:left-[2] top-1/2 -translate-y-1/2 bg-black/40 p-2 rounded-full text-amber-200 hover:text-orange-900 transition z-10" onClick={() => scrollByWidth('left')} >
                                <FaChevronLeft size={20} />
                            </button>
                            <button className="absolute cursor-pointer right-[0] md:right-[2] top-1/2 -translate-y-1/2 bg-black/40 p-2 rounded-full text-amber-200 hover:text-orange-900 transition z-10" onClick={() => scrollByWidth('right')} >
